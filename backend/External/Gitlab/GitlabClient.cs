...
namespace OpenData.External.Gitlab
{
    public class GitlabClient : IGitlabClient
    {
        ...
        public GitlabClient(IHttpClientFactory httpClientFactory)
        {
            ...
            _gitlabHost = "http://gitlab.potrik.com";
        ...
        private async Task<GitlabResponse<T>> _GetGitlabObject<T>(string endpoint)
        {
            try {
                var httpResponse = await _gitlabApiHttpClient.GetAsync(endpoint);
                var content = await httpResponse.Content.ReadAsStringAsync();
                if (httpResponse.IsSuccessStatusCode) return new GitlabResponse<T>(JsonSerializer.Deserialize<T>(content));
                else return new GitlabResponse<T>(content);
